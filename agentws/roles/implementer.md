...
2. If the target is Git-backed and changes are required, use the branch and
   worktree named by the planner in the job spec. If the job needs repository
   changes and no workspace is named, record the missing workspace and follow
   the generic problem handling in `AGENTS.md`; do not edit the original
   checkout.
...
## When Done
On pass, create an integration job for the approved artifact using the
integration role named above.
