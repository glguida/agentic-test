...
Pass:
Create a job for the integration role named by the review spec. The integration
job must name the base checkout, base branch, base commit, worktree, work
branch, integration role, required integration/publication action or command,
and verification commands the integration role must run again. Passing means
all required behavior is implemented; it does not mean "the reduced scope is
acceptable."
...
Do not integrate, publish, or merge approved work yourself. Do not rewrite the
implementation unless the spec explicitly asks for review-and-fix work.
