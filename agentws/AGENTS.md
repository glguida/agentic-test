...
AgentWS tools under `bin/` are the interface to task and job state. The tools
may store data locally, remotely, or through another system, and their output may
mention that mechanism. Treat those details as task context only.

Do not bypass the AgentWS tools, edit queue machinery by hand, or debug/repair
the AgentWS backend while doing a normal project job. If a tool or backend
mechanism fails, record the exact command and output, create a planner
notification job on the same task, comment on the task, then fail or release the
current job according to the problem-handling rules.
...
Use `bin/task-show <task-id>` for the current task view, and use task commands
for comments, state, and final result.
...
continue with implementation, review, or integration work; create the
required planner notification and fail or release the job with the concrete
reason.
...
Each job is available as a directory under `jobs/`:
...
When `server.url` exists, the remote job server is the source of truth and this
directory is a local mirror written by `bin/job-*` commands after claim, start,
log, done, fail, or release. Still read the local files above, and still use
only `bin/job-*` commands to change job state.

## Task Layout

Each task is accessed through `bin/task-*`. A job can finish without completing
the task. The task is complete only when a planner decides the overall task is
complete and records the result with:
...
bin/task-state <task-id> pr-sent -m "PR: <url>"
bin/task-result <task-id> <result-file>
bin/task-list
```

Do not mutate task state directly. Use `bin/task-*` commands.

The only task states are:

- `open`: the task is active.
- `pr-sent`: AgentWS has sent a PR or final external result.

When moving to `pr-sent`, use `bin/task-state`. The tool handles the backend
representation.
...
workflow: implementer -> reviewer -> <integration role>.
...
reviewer, and integration job specs.

Implementers, reviewers, and integration agents must work on the workspace named
by the planner. Do not make repository changes in the original project checkout
unless the task spec explicitly requires that.
...
reviewer inspects worktree and independently runs verification
        |
        | integration role integrates or publishes approved work
        v
integrated or published artifact
        |
        | integration role runs verification again
        v
...
Reviewer and integration jobs must include the verification commands to run. If
a verification command cannot be run, the role must record exactly why in the
job log and planner notification.
...
- `bin/job-log <job-id> [--agent-id <agent-id>] -s <summary> [-d <detail>]`
